...
(function ($, window, document) {
    var EZP = {
        init: function (options, elem) {
            ...
            //calculate window position to padding Left/ Top for background position when zooming
            self.zoomWindowPositionLeft = 0;
            self.zoomWindowPositionTop = 0;
        ...
        fetch: function (imgsrc, element, minZoom) {
            ...
            newImg.onload = function () {
                ...
                function setZoomWindowPosition() {
                    ...
                    const remainingHeight = self.options.zoomWindowHeight - newImg.height;
                    if (remainingHeight > 0) {
                        const positionTop = remainingHeight / 2;
                        if (positionTop > 0) {
                            self.zoomWindowPositionTop = Number(positionTop);
                        }
                    }
                    const remainingWidth = self.options.zoomWindowWidth - newImg.width;
                    if (remainingWidth > 0) {
                        const positionLeft = remainingWidth / 2;
                        if (positionLeft > 0) {
                            self.zoomWindowPositionLeft = Number(positionLeft);
                        }
                    }
                }
                //calculate window position to padding Left/ Top for background position when zooming
        ...
        startZoom: function () {
            ...
            function getWindowZoomStyle() {
                return {
                    ...
                    overflow: 'hidden',
                    zIndex: 100,
                    'box-shadow': self.options.zoomWindowBoxShadow ? 'rgba(0, 0, 0, 0.25) 0px 6px 8px 2px' : null
                };
            }
        ...
        setWindowPosition: function (e) {
            ...
            if (self.options.zoomType === 'window' || self.options.zoomType === 'inner') {
                ...
                if (self.options.easing) {
                    ...
                    if (!self.loop) {
                        self.loop = setInterval(function () {
                            ...
                            if (self.scrollingLock) {
                                ...
                                self.zoomWindow.css({
                                    backgroundPosition: '' + (self.windowLeftPos + self.zoomWindowPositionLeft) + 'px ' + (self.windowTopPos + self.zoomWindowPositionTop) + 'px'
                                });
                                self.scrollingLock = false;
                                self.loop = false;
                                ...
                                self.zoomWindow.css({
                                    backgroundPosition: '' + (self.xp + self.zoomWindowPositionLeft) + 'px ' + (self.yp + self.zoomWindowPositionTop) + 'px'
                                });
                            }
                        }, interval);
                    }
                    ...
                    self.zoomWindow.css({
                        backgroundPosition: '' +
                            (self.windowLeftPos + self.zoomWindowPositionLeft) + 'px ' +
                            (self.windowTopPos + self.zoomWindowPositionTop) + 'px'
                    });
                }
            }
        },
