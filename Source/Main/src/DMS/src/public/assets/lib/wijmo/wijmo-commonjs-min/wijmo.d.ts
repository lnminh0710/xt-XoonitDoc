 ...
 * let msg = wijmo.format('Hello {name}, you won {amount:n2}!', data);
 * ```
 *
 ...
 * let num = 42;
 * let dt = new Date();
 * let msg = glbz`the number is ${num}:n2, and the date is ${dt}:'MMM d, yyyy'!`;
 * ```
 */
...
export declare class Globalize {
     ...
     * ```
     *
     * The format string may also include constant prefix and a suffix
     * strings to be added to the output. If present, the prefix and
     * suffix are specified as double-quoted strings at the start and
    ...
    private static _parseNumericFormat;
