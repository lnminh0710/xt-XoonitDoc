...
import { FileUploader, FileItem, FileType } from '@app/shared/components/xn-file';
...
export class FileUploadComponent extends BaseComponent implements OnInit, OnDestroy {
  ...
  @Output() public onCloseDialog: EventEmitter<any> = new EventEmitter();
  ...
  public ngOnInit() {
    this.initUploader(false);
    this.initXlsx();
    this.inputId = this.widgetId + '-input-uploader';
  }
  ...
  private getUploadFileUrl(mode?: UploadFileMode, subFolder?: string): string {
    let url = Uti._serviceUrl.uploadFileExplorer + '?';
  ...
  private getUploadUrl(): string {
    return this.getUploadFileUrl(this.uploadFileMode, this._idFolder);
  }
  ...
  public openFileExplorerDialog(event) {
    console.log('openFileExplorerDialog', this.uploader);
    if (this.uploader && this.uploader.isUploading) {
      this.modalService.createModal(this.messageWarnOption);
      this.modalService.showModal();
      return;
  ...
  public getFileSize(total: number, type?: number) {
    if (!type) type = 0;
    ...
    if (total >= 1024) {
      ...
      type += 1;
      if (count > 1024) {
        return this.getFileSize(count, type);
      }
    }
    const result = count.toFixed(1);
    switch (type) {
      case 0:
        return result + ' byte';
      case 1:
