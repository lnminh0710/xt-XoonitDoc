...
import { FolerTreeChange, FolerTreeExpandCollaspe } from '@app/state-management/store/actions/app/app.actions';
...
@Component({
    selector: 'widget-email-folder-tree',
    templateUrl: 'widget-email-folder-tree.component.html',
    styleUrls: ['widget-email-folder-tree.component.scss'],
    changeDetection: ChangeDetectionStrategy.OnPush,
})
export class WidgetEmailFolderTreeComponent
    ...
    implements OnInit, AfterViewInit, OnDestroy, OnChanges
{
    ...
    private _findDocumentSelected() {
        const { idDocumentContainerScans, idDocumentTree, il } = this._params;
        if (!idDocumentContainerScans || !idDocumentTree) return;

        const idDocTree = Number(idDocumentTree) === IdDocumentTreeConstant.EMAIL ? il : idDocumentTree;
        const selectedFolder = this.xnDocumentTree.selectDocumentFolder(parseInt(idDocTree), true);
        ...
        const contextMenuActions: Array<ContextMenuAction> = [
            new ContextMenuAction({
                click: () => {
                    ...
                    } else idDocumentParentAfterCallApi = 2; // 2 is default email tree

                    event.treeComponent.treeModel.virtualRoot.data.idDocumentParentAfterCallApi =
                        idDocumentParentAfterCallApi;
                    event.treeComponent.treeModel.virtualRoot.data.idDocumentParent = 2; // 2 is default email tree
                    event.treeComponent.treeModel.virtualRoot.data.idDocumentType = 6; // 6 is email tree
        ...
        if (event.node?.data?.canEdit) {
                ...
                new ContextMenuAction({
                    click: () => {
                        if (event.node.data.isCompany || event.node.data.isUser)
                            event.node.data.idDocumentParentAfterCallApi = IdDocumentTreeConstant.EMAIL;
            ...
            if (!event.node.data.isCompany && !event.node.data.isUser) {
                contextMenuActions.push(
                    new ContextMenuAction({
                        click: () => {
                            if (event?.node?.data?.children) event.node.data.children = null;
    ...
    public onExpandCollapse(event) {
        this.store.dispatch(new FolerTreeExpandCollaspe({ node: event?.node }));
    }
