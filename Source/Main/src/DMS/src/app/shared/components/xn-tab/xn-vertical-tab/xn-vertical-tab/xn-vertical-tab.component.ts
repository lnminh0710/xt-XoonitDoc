...
import { XnVerticalTabHeaderComponent } from '../xn-vertical-tab-header';
...
@Component({
    selector: 'xn-vertical-tab',
    styleUrls: ['./xn-vertical-tab.component.scss'],
    templateUrl: './xn-vertical-tab.component.html',
    host: {
        '(contextmenu)': 'onRightClick($event)',
    },
})
export class XnVerticalTabComponent extends BaseComponent implements OnInit, OnDestroy {
    ...
    public editLayout: boolean = false;
    private dragProgressTimer = null;
    ...
    private documentThumnailOpenUnknownDocumentType: Subscription;
    ...
    ngOnInit() {
        ...
        this.subscribeOpenUnknowDocumentType();
        this.subscribeToggleOpenCloseDocumentFolderTree();
    ...
    subscribeToggleOpenCloseDocumentFolderTree() {}
    ...
    private subscribeOpenUnknowDocumentType() {
        this.documentThumnailOpenUnknownDocumentType = this.dispatcher
            .pipe(filter((action) => action.type === DocumentThumbnailActions.DOCUMENT_THUMBNAIL_SELECT_UNKNOWN_ITEM))
            .subscribe((action) => {
                this.toggleLeftArea(true);
            });
    }
    ...
    public dragEnd(event: any) {
        this.splitterConfig = {
            ...
            subRightLeftHorizontal: this.subRightHorizontalSplit.areas[0].size,
            subRightRightHorizontal: this.subRightHorizontalSplit.areas[1].size,
        };
                ...
                if (data && data.length) {
                    ...
                    if (verticalTabSplitterSettings && verticalTabSplitterSettings.length) {
                        verticalTabSplitterSettings.forEach((setting) => {
                            ...
                            // this.subRightHorizontalSplit.updateArea(this.subRightHorizontalSplit.areas[0].component, 1, this.splitterConfig.subRightLeftHorizontal, 20);
                            // this.subRightHorizontalSplit.updateArea(this.subRightHorizontalSplit.areas[1].component, 1, this.splitterConfig.subRightRightHorizontal, 20);
                        });
    ...
    private stylingSplitGutter() {
        if (this.horizontalSplit) {
            ...
            let gutter = $('.as-split-gutter', this.horizontalSplit.elRef.nativeElement);
            if (gutter.length) {
            ...
            if (this.horizontalSplit.displayedAreas && this.horizontalSplit.displayedAreas.length) {
                let areas = $('as-split-area', this.horizontalSplit.elRef.nativeElement);
                if (areas.length) {
    ...
    private toggleCapturedForm(isEnabled: boolean) {
        if (isEnabled) {
            this.splitterConfig.subRightLeftHorizontal = 0;
            this.splitterConfig.subRightRightHorizontal = 100;

            // this.subRightHorizontalSplit.updateArea(this.subRightHorizontalSplit.areas[0].component, 1, 0, 20);
            return;
        }
        ...
        // this.subRightHorizontalSplit.areas[0].sizeUser = this.splitterConfig.subRightLeftHorizontal;
        // this.subRightHorizontalSplit.areas[1].sizeUser = this.splitterConfig.subRightRightHorizontal;
