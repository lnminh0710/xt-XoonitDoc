import { CLASS_LOADING, CLASS_TRANSITION, EVENT_LOAD, EVENT_TRANSITION_END, EVENT_VIEWED } from './constants';
import {
    addClass,
    addListener,
    assign,
...
export default {
    ...
    initImage(done) {
        ...
        sizingImage = getImageNaturalSizes(image, options, (naturalWidth, naturalHeight) => {
            ...
            width = Math.max(width * 0.9, naturalWidth);
            height = Math.max(height * 0.9, naturalHeight);
            ...
            const initialImageData: any = assign({}, imageData);
