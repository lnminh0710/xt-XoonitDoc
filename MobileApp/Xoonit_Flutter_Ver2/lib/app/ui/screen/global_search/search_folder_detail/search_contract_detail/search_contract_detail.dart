...
class SearchContractDetailScreen extends StatefulWidget {
  const SearchContractDetailScreen({
    Key key,
  ...
  @override
  _SearchContractDetailScreenState createState() =>
      _SearchContractDetailScreenState();
}

class _SearchContractDetailScreenState
    extends State<SearchContractDetailScreen> {
  ScrollController _controller;
  DashBoardBloc _dashBoardBloc;
  SearchContractDetailBloc _searchContractDetailBloc;
  ...
  _scrollListener() {
    if (_controller.offset >= _controller.position.maxScrollExtent &&
      ...
      _searchContractDetailBloc.loadMoreSearchContractDetail();
    }
  ...
  Widget build(BuildContext context) {
    _searchContractDetailBloc = BlocProvider.of(context);
    _dashBoardBloc = BlocProvider.of(context);
    return StreamBuilder<ContractSearchDetailResponse>(
      stream: _searchContractDetailBloc.searcContractResponseStream,
                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      children: <Widget>[
                            ...
                            ? GridView.builder(
                                ...
                                gridDelegate:
                                    SliverGridDelegateWithFixedCrossAxisCount(
                                        ...
                                        crossAxisSpacing: 12),
                                itemBuilder: (_, index) {
                                  SearchContractResult contractResult =
                                      snapshot.data.item.results[index];
                                  setupColumnSetting(snapshot.data, index);
                                  return SearchDetailItem(
                                    onItemClicked: () {
                                      _searchContractDetailBloc.reviewDocument(
                                          context,
                                          snapshot.data.item.results[index], _dashBoardBloc);
                                    },
                                    thumbnailURL: GeneralMethod.getThumbnailURL(
                                        contractResult.scannedPath,
                                        contractResult.scannedFilename),
                        ...
                        StreamBuilder<bool>(
                            stream: _searchContractDetailBloc.loadingStream,
  ...
  void setupColumnSetting(
      ContractSearchDetailResponse searchContractDetailResponse, int index) {
    _captureDetailKey.clear();
    _captureDetailValue.clear();
    SearchContractResult contractResult =
        searchContractDetailResponse.item.results[index];
    List<ColumnSearchSettings> columnSearchSettings =
        columnSearchSettingsFromJson(
            searchContractDetailResponse.item.setting[0][0].settingColumnName);
    ...
    if (settings != null) {
      settings.forEach((element) {
         if (element.setting != null &&
          ...
          _captureDetailValue
              .add(contractResult.toJson()[columnKey].toString() ?? '');
        }
      });
    }
