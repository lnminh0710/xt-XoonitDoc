...
import 'package:caminada/app/ui/component/no_item_found.dart';
...
typedef TextFieldBuilder = Widget Function(TextEditingController controller,
    FocusNode focus, Function(String) onTextChange);
...
class SearchWidget<T> extends StatefulWidget {
  ...
  final Future<List<T>> Function(String text) queryBuilder;
...
class MySingleChoiceSearchState<T> extends State<SearchWidget<T>> {
  ...
  Future<void> onTextChange(String text) async {
    if (text.trim().isNotEmpty) {
      final filterList = await widget.queryBuilder(text);
      if (filterList == null) {
        throw Exception(
  ...
  Widget build(BuildContext context) {
    ...
    textField = widget.textFieldBuilder != null
        ? widget.textFieldBuilder(_controller, _focusNode, (text) {
            onTextChange(text);
          })
        : Padding(
  ...
  void onTap() {
    ...
    final width = Dimension.getWidth(1);
